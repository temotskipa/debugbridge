...
26.2-snapshot-3 does not use this path. It runs against Mojang-named classes directly and skips mapping download/remap entirely.
...
Output JARs are in `mod/fabric-*/build/libs/`, including `mod/fabric-26.2-snapshot-3/build/libs/`.
...
- **Outbound**: Mapped builds may make an HTTPS request at startup to fetch Mojang mapping metadata from `piston-meta.mojang.com` and `launcher.mojang.com` (official Mojang APIs); 26.2-snapshot-3 skips that path
...
- **26.2-snapshot-3**: Unobfuscated; uses Mojang names directly and does not download or remap mappings
